...
// Laundry item components (List & Wardrobe)
const LaundryItem = ({ item, onWear, onWash, onDelete }) => {
  ...
  return (
    <div className={`p-4 rounded-xl flex items-center justify-between transition-all duration-300 ${needsWashing ? 'bg-red-50 border-red-200' : 'bg-white border-slate-200'} shadow-sm border`}>
      <div className="flex-1 flex items-center space-x-4">
        ...
        <div className="flex-1 space-y-1">
          ...
          <div className="flex items-center space-x-2 text-sm text-slate-500">
            ...
            <div className={`px-2 py-0.5 rounded-full text-xs font-semibold ${getCategoryColor(item.category)}`}>
      ...
      <div className="flex space-x-2">
        <button onClick={() => onWear(item)} className="p-2 bg-indigo-500 text-white rounded-full hover:bg-indigo-600 transition-colors duration-200 transform hover:scale-105">
          <Plus size={20} />
        </button>
        <button onClick={() => onWash(item)} className="p-2 bg-green-500 text-white rounded-full hover:bg-green-600 transition-colors duration-200 transform hover:scale-105">
          <CheckCircle size={20} />
        </button>
        <button onClick={() => onDelete(item)} className="p-2 bg-slate-200 text-slate-600 rounded-full hover:bg-slate-300 transition-colors duration-200 transform hover:scale-105">
          <Trash size={20} />
        </button>
      </div>
    </div>
...
const ConfirmationModal = ({ message, onConfirm, onCancel }) => (
  <div className="fixed inset-0 bg-slate-900 bg-opacity-50 flex items-center justify-center p-4">
...
const App = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-900 font-sans p-4 flex flex-col items-center">
      <div className="w-full max-w-2xl bg-slate-200 p-6 rounded-3xl shadow-2xl space-y-8 border border-slate-300">
        <header className="text-center space-y-2">
          <h1 className="text-4xl font-extrabold text-slate-800">Laundry Tracker</h1>
          <p className="text-slate-600 text-lg">Track clothes usage to know when to wash them.</p>
        </header>
        ...
        <form onSubmit={addItem} className="flex flex-col space-y-4">
          <input type="text" value={itemName} onChange={e=>setItemName(e.target.value)} placeholder="Item Name" className="p-3 rounded-xl border"/>
          <input type="text" value={itemImageUrl} onChange={e=>setItemImageUrl(e.target.value)} placeholder="Image URL" className="p-3 rounded-xl border"/>
          <select value={selectedCategory} onChange={e=>setSelectedCategory(e.target.value)} className="p-3 rounded-xl border">
